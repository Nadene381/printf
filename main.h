...
int (*getFunct(const char *s, int index))(va_list, char *, unsigned int);
int printFunc(const char *s, int index);
unsigned int handleBuffer(char *buf, char c, unsigned int ibuf);
int print_buf(char *buf, unsigned int nbuf);
char *binaryArray(char *binary, long int int_in, int isneg, int limit);
char *OctArray(char *bnr, char *oct);
char *longOctArray(char *bnr, char *oct);
